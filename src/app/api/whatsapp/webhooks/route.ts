  ...
  try {
    ...
    try {
      ...
      if (ccP?.patientId) {
        ...
        if (prov?.id) {
          isProvider = true
          const providerDisplayName = String((prov as any).user?.name || '').trim() || null
          const vis = await prisma.visitor.findFirst({ where: { contacts: { some: { type: 'whatsapp', value: phoneE164 } } }, select: { id: true, displayName: true } })
          if (vis) {
            visitorId = vis.id
    ...
    try {
      ...
      if (incomingText2) {
        if (isPatient && patientId) {
          ...
          await prisma.commMessage.create({ data: { conversationId: convo.id, direction: 'inbound', via: 'whatsapp', body: incomingText2, senderType: isProvider ? 'provider' : 'visitor', senderId: visitorId, meta: { messageId } } })
        }
      }
    } catch {}
