...
async function resolvePatientWhatsapp(patientId: string): Promise<string | null> {
  const cc = await (prisma as any).contactChannel.findFirst({ where: { ownerType: 'patient', patientId, type: 'whatsapp', consent: true }, orderBy: { preferred: 'desc' }, select: { value: true } })
