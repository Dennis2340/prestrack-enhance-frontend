...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const toE164 = await resolveVisitorWhatsapp(id)
    if (!toE164) return NextResponse.json({ error: 'visitor has no whatsapp' }, { status: 400 })

    // ensure conversation exists
    let convo = await prisma.conversation.findFirst({ where: { subjectType: 'visitor' as any, visitorId: id }, orderBy: { updatedAt: 'desc' } })
    if (!convo) {
      convo = await prisma.conversation.create({ data: { subjectType: 'visitor' as any, visitorId: id, channel: 'whatsapp' as any, status: 'open' as any, lastMessageAt: new Date() } })
    }
